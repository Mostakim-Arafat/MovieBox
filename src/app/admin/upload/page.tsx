...
export default function MovieUploadPage() {
  ...
  const [videoUrl, setVideoUrl] = useState("https://drive.google.com/uc?export=download&id=1p7IlG5yonk01FdU07x37SxH9cWw_DJ7m");
 ...
 // Form Submit Handler
  const handleFormSubmit = async (e: React.FormEvent, submitStatus: "Published" | "Draft") => {
    ...
    setUploadStage("Initializing Mux asset creation...");

    try {
      // 2. Step 1: Send videoUrl to Mux API
      setUploadProgress(40);
      setUploadStage("Sending URL to Mux for processing...");
      ...
      if (!muxRes.ok) {
        ...
        throw new Error(errorData.error || "Failed to create Mux asset.");
      }

      const { muxAssetId, muxPlaybackId } = await muxRes.json();

      // 3. Step 2: Save full movie details along with Mux IDs into MongoDB
      ...
      const moviePayload = {
        ...
        cast: castList,
        status: submitStatus,
        videoUrl, // Original source URL
        muxAssetId,   // Generated by Mux
        muxPlaybackId // Used by Mux Player on frontend
      };
      ...
      if (!response.ok) {
        throw new Error("Failed to save movie to catalog.");
      }
